...
GIT_REMOTE=${PWD}/../hillslope-final
...
svn checkout --ignore-externals --quiet ${SVN_ROOT_URL}/branch_tags/hillslope_hydrology_tags/${TAGS[0]}/${REMAP} ${SVN_REPO} >> ${SVN_OUTPUT} 2>&1

for tag in ${TAGS[@]}; do
    echo "Working on tag: ${tag}" >> ${RESULTS}

    pushd ${SVN_REPO}
    svn switch --accept theirs-full --ignore-externals --quiet ${SVN_ROOT_URL}/branch_tags/hillslope_hydrology_tags/${tag}/${REMAP} >> ${SVN_OUTPUT} 2>&1
    popd
